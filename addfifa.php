 ...
 */
    ...
    if($num_row == 1)
    {
        ...
        if($num_row2 == 1)
        {
            //update player 1 FIFA score
            $sql3 = "update gameData set FIFA_SCORE = FIFA_SCORE + '$p1s' where ID = '$p1ID' ";
            ...
            if($num_row3 == 1)
            {
                ...
                //update player 2 FIFA score
                $sql4 = "update gameData set FIFA_SCORE = FIFA_SCORE + '$p2s' where ID = '$p2ID' ";
                ...
                if($num_row4 == 1)
                {
                    ...
                    $sql12 = "insert into gamehistory(P_ID1, P_ID2, GAME, P_SCORE1, P_SCORE2, ADD_DATE)
                                          values('$p1ID', '$p2ID', 'FIFA', '$p1s', '$p2s', '$dateAdd')";
                    $res12 = mysqli_query($con, $sql12);
                    //check if player 1 wins
                    if($p1s > $p2s)
                    {
                        //update player 1 FIFA wins
                        $sql5 = "update gameData set FIFA_WINS = FIFA_WINS + '$win' where ID = '$p1ID'";
                        ...
                        if($num_row5 == 1)
                        {
                            //update player 2 FIFA loss
                            $sql6 = "update gameData set FIFA_LOSS = FIFA_LOSS + '$loss' where ID = '$p2ID'";
                    ...
                    {
                        //update player 1 FIFA loss
                        $sql8 = "update gameData set FIFA_LOSS = FIFA_LOSS + '$loss' where ID = '$p1ID'";
                        ...
                        if($num_row8 == 1)
                        {
                            //update player 2 FIFA wins
                            $sql9 = "update gameData set FIFA_WINS = FIFA_WINS + '$win' where ID = '$p2ID'";
